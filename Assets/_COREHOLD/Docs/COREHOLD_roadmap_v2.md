...
| 8 | Floor fit | R11/R26 | ground sized from the camera frustum, then the pack's `groundPrefab`/`groundMaterial` applied and tiling recomputed for that size |
...
**Packs live in `Assets/_COREHOLD/Data/EnvPacks/`; their prefab pool lives outside the game folder.** `Assets/Authoring/EnvPack/{Landmarks, MidField, Clutter, Silhouettes}` — where you file a prefab *is* its role, scanned recursively, and `Build Env Pack From Folders` turns the tree into a measured pack. The pool sits outside `_COREHOLD/` because it is generation-time input rather than shipped game content: `_COREHOLD/` is what the game *is*, `Authoring/` is what levels can be built *from*.
