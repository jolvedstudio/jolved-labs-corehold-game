...
**As shipped.** `WaveMutator` is a `[Flags]` enum on `WaveDefinition` (compose freely; `None` = vanilla, and every shipped/generated wave stays `None` — nothing was authored on). Values are `[TUNE]` fields on the WaveManager (Storm ×1.3 air speed, Overcharge ×1.3 HP / ×1.5 bounty, Blackout distance ×2). Mutators are derived from the WAVE NUMBER at the moment each unit actually spawns (`MutatorsForWave`), never threaded through spawn state, so pending-queue admissions seconds later read the same answer. **Storm** stamps a fourth mover speed slot (`WaveSpeedMultiplier` — base × enrage × status × wave, no slot overwrites another). **Convoy** = two moves: all ground groups collapse onto the first group's resolved spawner (after the R40 siege rotation, so siege maps still rotate per wave), and each unit passes `forceSingleLane` INTO `EnemyMover.Configure` — a Configure input, not a settable property, because registration happens inside that call — which routes it through RouteTraffic's existing WIDE-BODY path: every lane occupied, centreline render, which is exactly single-file ordering with zero new scheduling machinery. **Overcharge** multiplies HP and bounty at the existing spawn-time scalar sites. **Blackout** stamps `Enemy.AcquisitionDistanceScale = 2`; `TowerTargeting.InRange` scales the MAX-range comparison only — an unlit unit is seen at half range, but min-range dead zones stay physical so darkness can never let a Mortar fire inside its own dead zone. The ticket's "acquisition range ×2" was read as "distance counts double" (towers see unlit units at HALF range) — the only reading that makes Blackout harder and gives R24's Floodlight something to restore; R24 will insert the lit-check at the same stamp/compare sites. With no floodlights yet, every Blackout unit is unlit — globally visible pressure. DebugConsole `T` cycles a forced set (None → Storm → Convoy → Overcharge → Blackout → All) OR-ed into waves started after the press, so every mutator is testable without authoring wave assets.
...
**As shipped.** Rank state (`KillCount`, `VeterancyRank` 0–3, thresholds 25/75/150, +4%/rank as `[TUNE]` constants) lives on `Tower`, per-instance. Kill attribution lives AT THE DAMAGE SITES, not on Enemy: `TowerWeapon.ApplyDamage` (hitscan + every Arc chain hop) and `Projectile.ApplyDamage` (primary and every splash victim) check alive→dead across `TakeDamage` and credit the owning tower — `Projectile.Spawn/Launch` gained an optional `owner` param, and `Enemy` never learns what a `Tower` is. Debug kills (`K`), leaks and watchdog culls credit nobody. The multiplier composes into the existing damage path in both places it is computed: `Tower.EffectiveDamage`/`EffectiveDps` (what the panel shows) and `TowerWeapon`'s per-mount `effectiveDamage` (what shots deal), as `(1 + aura) × (1 + 0.04·rank)`. **Sell forfeits rank** structurally: rank dies with the instance, and `Build()` also resets it explicitly so a future pooled/reused tower instance can never inherit a career; upgrades go through `SetTier` and keep it. Chevrons are drawn by OverlayManager as a third pooled layer diffed against `Tower.Live` per frame (same doctrine as enemy overlays — no per-tower component, no event wiring), rendered as bold amber CARETS (`^`..`^^^`, glyphs guaranteed in the UI font, unlike U+25B2) at 8.4 m — above the tower health bar at 6.8. Rank-ups also fire the pooled build puff + a log line. No scene or asset change: everything attaches at runtime, so shipped and generated maps both get it with nothing to re-run.
...
**Scaffolded** (the CoPlay half; the human readability pass remains). `NightVariant` runtime component: night = a LIGHTING STATE (ambient → 0.25 + flat night colour, sun dimmed/cooled to moonlight, emissive materials ×1.7 via MaterialPropertyBlocks — shared assets never mutated, enemy renderers excluded because enrage owns their MPBs, re-scanned each toggle so new towers join), all cached-and-restored exactly so the toggle is idempotent both ways. `Tools → COREHOLD → Scene Setup → Night Variant` scaffolds a "NightVariant" root with ≤10 non-shadowing lamps auto-placed for readability (warm over the Core, cool at each route mouth, the rest at ⅓/⅔ marks along the routes), day-active by default. **Extended to every map on request:** the tool works on the ACTIVE scene (routes via `SceneQuery.InActiveScene`, so a second loaded scene can never leak lamps in), the generation pipeline's weather stage calls it for every emitted map, and "NightVariant" is registered in `SceneContainers.Groups` under `_Rendering` so the grouping verify pass stays clean. DebugConsole `N` toggles. The R24 Floodlight's own lamp and boosted lens compose with it for free. Known interplay: the WeatherApplier also tints ambient — night is last-writer-wins; toggle night after weather, or re-toggle. **Human half open:** play at 907×510, press `N`, nudge/recolour lamps until the legibility bar holds, then decide where "selectable" surfaces for players (menu toggle is R32/menus territory).
...
**As shipped.** The light radius RIDES `TowerTier.auraRadius` (12, zero aura bonuses): the SupportAura pass sees a relay granting nothing (a no-op under max-per-axis), `IsSupportRelay` classifies it correctly everywhere for free (never acquires, never buffed, support-style panel), and a future tiered floodlight is data-only. `Floodlight` mirrors the SupportAura component pattern — static `Lights` registry in OnEnable/OnDisable, no per-frame work; `IsLit(pos)` is planar XZ (the beam is a column: a Wasp over a lamp is lit, same convention as the R19 burst). The Blackout hook sits in `TowerTargeting.AcquisitionScale`: units with scale 1 never query the registry at all, stamped units get scale 1 back while lit. Chassis is PROCEDURAL and committed (primitives + three generated materials under `Art/Materials`, no Vendor references): plinth/mast/head/emissive lens, a 12 m translucent ground disc (the gameplay-readable lit circle — geometry, not a light cookie), and a real non-shadowing point light so the R23 night variant gets working lamps for free. `Tools → COREHOLD → Scene Setup → Floodlight Tower` authors `Tower_Floodlight.asset` + prefab and wires them (also appended to WireTowerPrefabs). Build-menu geometry widened for six entries (panel 760→904, row 736→880). **Run order matters: Floodlight Tower first, THEN Build Real UI** (the theme catalogue bakes the def list at build time); the icon tool picks the new definition up on its next run (menu shows name/cost/LIGHT tag meanwhile). Generated maps need nothing — the pipeline's existing BuildRealUI call sees the new definition once the asset exists.
