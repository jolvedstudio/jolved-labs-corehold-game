...
- New visual effect → add an entry to the **`VFXDirector.Effect`** enum (pooled) and play via **`VFXDirector.Instance.Play(Effect, pos[, forward|rotation, scale])`**, or the typed helpers `PlayMuzzle` / `PlayImpact` / `PlayExplosion(pos, splashRadius)` / `PlayEnemyDeath` / `PlayCoreHit`.
...
- World-space overlays (rings, chevrons, banners anchored in the world) → **`OverlayManager`**.
...
`Pin: @VFXDirector.cs @GameManager.cs @Main Camera`
Codify a reusable, subtle impact standard used across the game: a directional camera kick with rapid exponential decay and an optional micro hit-stop, exposed as helpers so future tickets reuse them (never re-implement). Add the kick as a method the `Main Camera` framing respects, and gate intensity behind `[TUNE]` fields (default: small kick on `PlayImpact`/`PlayCoreHit`, larger on `PlayExplosion`). Keep it readability-first: exponential decay back to the framed position, no motion-sickness-inducing random jitter, and an accessibility `[TUNE]` scalar (0 = off).
...
Add a serialized `bool useSpline` (default OFF) to `PathRoute`. In `Recompute()`, when `useSpline`, build a `UnityEngine.Splines.Spline` from the `waypoints` positions as `TangentMode.AutoSmooth` knots, wrap it in a cached `NativeSpline` and build an arc-length table; delegate `SamplePosition(distance, out tangent)` and `Length` to the spline path. Because `Recompute()` is called from `Awake`, `OnValidate`, **and every `OnDrawGizmos`**, guard it with a dirty-check keyed on a hash of waypoint positions so the spline/native/arc-table rebuild only when waypoints actually move — **zero GC allocation per frame** in steady state. Air units are untouched (EnemyMover air path stays route-free straight flight to Core at flightAltitude). Update the gizmo to draw polyline vs. spline overlay for comparison.
...
The west and north routes carry duplicated, identical-position knots for the shared 120 m snake tail, but AutoSmooth derives each knot's tangent from its neighbours — and the pre-merge neighbours differ — so the two splines diverge at the merge. Fix by pinning **identical explicit tangents** at the merge knot on both routes (or, if cleaner, start the shared geometry one knot earlier so both routes share an identical pre-merge neighbour). Add an editor check that samples both route splines every 0.5 m over the shared tail and reports maximum divergence.
...
Add a status-effect list (a struct list) on `Enemy` for stun and slow. Rules: **refresh-not-stack** (re-applying refreshes duration, doesn't add), effects modify a base-speed read in `EnemyMover` (never mutate the stored base). Add `stunResistance` to `EnemyDefinition` (Colossus = 25% duration reduction). Show status via `OverlayManager`/`VFXDirector` (new pooled `Effect.Stun` / `Effect.Slow`), not a new component per enemy.
**Done when:** stun halts movement and slow scales speed, both refresh rather than stack, base speed is never corrupted, Colossus takes 25% reduced stun duration, and status VFX are pooled through VFXDirector. Re-run the balance model before tuning (stun uptime is a model term — R22).
...
Generate a candidate superset of ~20 hardpoint positions offset from route bends at ≥3.75 m from the centreline. Score every candidate once with the curve sampler per `TurretKind`. Classify: **Premium** (4+ covered spans), **Standard** (2–3), **Rear** (near-Core + air-terminal), **Overwatch** (sparse, long sight-line). Discard any <2 spans. Run **one deterministic greedy selection pass** by class need, tie-broken by score, enforcing a minimum inter-pad spacing (~6 m, `[TUNE]`).
...
On successful generation, emit a `LevelDefinition`: clone `rulesTemplate`, then **solve `hpGrowthPerWave` so margins match the shipped map's band** (use the balance model as the solver), and set `maxLiveEnemies` from `RouteTraffic.DerivedCapacity(largestRadius)` for the generated routes. Wire spawners (index 0 west ground / 1 north ground / 2 air) via the existing `WireOne`.
...
Add a score for each run (function of waves cleared, integrity remaining, streaks, time, difficulty). Persist a **local leaderboard** in the existing `SaveData` (per map + mode). On the `ResultScreen`, generate a **shareable code/string** stamped with the seed and score (the Spelunky/Slay-the-Spire "same seed, share text" pattern) so players can challenge friends on identical terms. **Do not trust any client-submitted remote score** — local + share code only here.
...
Define per-map medal criteria — **no-leak** (integrity untouched), **no-sell** (never sold a tower), **under-time** — evaluated at run end and feeding the R4 run-stats screen. Medals grant **stars**; stars unlock **cosmetic or starting-loadout variety only** (e.g., turret skins from the hand-painted set, alternate starting-salvage/loadout presets) — **never power, never P2W**. Persist stars/unlocks in `SaveData`. Include a free "respec"/reset for any spent stars.
...
## Caveats
- The public repo `github.com/jolvedstudio/jolved-labs-corehold-game` returned a 404 at fetch time (likely private or renamed); this roadmap is written against the supplied verified code facts, which are treated as ground truth. CoPlay validates against live code on every ticket, so exact enum names (`AudioDirector.Sfx`, `VFXDirector.Effect`) and `PathRoute` member signatures will be confirmed in-thread.
