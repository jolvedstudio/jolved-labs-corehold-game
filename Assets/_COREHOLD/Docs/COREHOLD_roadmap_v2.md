...
**Packs live in `Assets/_COREHOLD/Data/EnvPacks/`; their prefabs are categorised by folder.** `Assets/_COREHOLD/Prefabs/EnvPack/{Landmarks, MidField, Clutter, Silhouettes}` — where you file a prefab *is* its role, scanned recursively, and `Build Env Pack From Folders` turns the tree into a measured pack. Three rules make that safe to re-run:
