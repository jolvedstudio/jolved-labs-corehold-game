...
| Dust motes look huge, or precipitation is sparse | Your `Weather_*.asset` presets predate the R14 retune — they live in your project, not the repo, so code changes never touch them | Re-run **Tools → COREHOLD → Look → Setup Weather** once; it re-authors the presets in place |
