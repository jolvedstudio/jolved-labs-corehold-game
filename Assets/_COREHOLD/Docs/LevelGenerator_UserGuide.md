...
| `topology` | The map's SHAPE, as one named parameter. **Corridor** (shipped: two legs merge into a folded run), **SingleLane** (one entrance, no merge), **Pincer** (2 approaches, opposite), **Siege** (3 approaches, all sides), **Encirclement** (4 approaches over 270° — all sides but one). Sector count and arc are not separate knobs: each name is a combination measured to separate on a 130×75 field at 154 m, and most other combinations cannot generate at all. Parity is always Corridor. | Corridor |
